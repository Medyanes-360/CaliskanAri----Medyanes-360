  ...
  return (
    <>
      ...
      <button
        ...
        className={`${
          collapsedSidebar
            ? "text-secondary  bg-white rounded"
            : "text-white bg-secondary rounded"
        } hover:scale-110 transition-all ml-12 absolute left-0 top-[12px] z-40 flex gap-4 p-2`}
      >
        <span className="hidden sm:block">İşlem Paneli</span>
        <CgMenu size={25} />
      </button>
      ...
      {!collapsedSidebar && (
        <div
          onClick={() =>
            collapsedSidebar && setCollapsedSidebar(!collapsedSidebar)
          }
          className={`${
            collapsedSidebar
              ? "absolute hidden"
              : " absolute z-40 sm:relative sm:block"
          } w-full sm:w-[300px]`}
        >
          <div
            className={` ${
              collapsedSidebar ? "bg-primary" : "bg-white"
            } min-w-300px  sm:max-w-[300px] w-full min-h-[calc(100vh-64px)] max-h-[calc(100vh-64px)] overflow-y-auto`}
          >
            <div
              onClick={() => setCollapsedSidebar(!collapsedSidebar)}
              className={`${
                collapsedSidebar ? "bg-primary" : "bg-secondary"
              } cursor-pointer w-full  h-12 flex justify-center p-2 items-center`}
            >
              <div className="text-white font-bold flex justify-center items-center content-center">
                YÖNETİCİ PANELİ
              </div>
            </div>
            ...
            <div className="flex justify-between flex-col content-between w-full min-h-[calc(100vh-130px)]">
              <div
                className={`${
                  width < 640 && collapsedSidebar ? "hidden" : "block"
                }`}
              >
            ...
            */}
                ...
                {contentData &&
                  contentData.map((item, index) => {
                    !item.parentButton && item.index == 0 && (
                      <div
                        onClick={() =>
                          handeButtonClick(item.name, item.component)
                        }
                        key={index}
                      >
                ...
                <Button text={"Anasayfa İşlemleri"} icon={<AiFillHome />}>
                  <div
                    onClick={() =>
                      handeButtonClick("Genel Duyuru", <GeneralTopPageBanner />)
                    }
                  >
                  ...
                  <div
                    onClick={() =>
                      handeButtonClick("Slider Banner", <Slider />)
                    }
                  >
              ...
              <div
                className={`${
                  collapsedSidebar ? "hidden" : "block"
                } flex flex-col gap-4 justify-center items-center content-center border-t-2 py-4`}
              >
                <Link href={"/"}>
                  <Image
                    src={"/logo.png"}
