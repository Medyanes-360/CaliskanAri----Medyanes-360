...
import { informations as informationsinFile } from '../constants';
...
export const InfoSection = () => {
  ...
  useEffect(() => {
    ...
    if (informationsinLocalStorage) {
      if (informationsinFile.length !== informationsinLocalStorage.length)
        return;

      const concatenatedInformations = informationsinFile.map((item, index) => {
        return {
          ...item,
          ...informationsinLocalStorage[index],
        };
      });

      setInformations(concatenatedInformations);
    }
  }, []);
