...
import { coursesCard as coursesCardinFile } from '../constants';
...
export const TopCoursesCard = () => {
  ...
  useEffect(() => {
    ...
    if (coursesCardinLocalStorage) {
      if (coursesCardinFile.length !== coursesCardinLocalStorage.length) return;

      const concatenatedCoursesCard = coursesCardinFile.map((item, index) => {
        return {
          ...item,
          ...coursesCardinLocalStorage[index],
        };
      });

      setCoursesCard(concatenatedCoursesCard);
    }
  }, []);
