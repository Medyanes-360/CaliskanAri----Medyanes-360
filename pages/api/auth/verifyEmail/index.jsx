  ...
  if (req.method !== 'POST') {
    ...
    if (!session) {
      try {
        if (!email) {
          throw new Error(
            'Lütfen Email Adresinizin Doğru Olduğundan Emin Olun.'
          );
        }

        if (!key || !role) {
          throw new Error(
            'Mail Doğrulama Sırasında Bir Hata Oluştu. Lütfen Tekrar Deneyin.'
          );
        }
        ...
        const { error, status, message } = await VerifyEmail({
          key,
          email,
          role,
        });
